...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['id_orden']) && isset($_POST['motivo_rechazo']) && isset($_POST['estado'])) {
    // Validar token CSRF
    ...
    } elseif (strlen($motivo_rechazo) < 10) {
        $errores[] = "El motivo del rechazo debe tener al menos 10 caracteres";
    } elseif (strlen($motivo_rechazo) > 500) {
        $errores[] = "El motivo del rechazo debe tener menos de 500 caracteres";
    }

    $tipos_estado_permitidos = ['rechazada'];
    if (empty($estado)) {
        $errores[] = "El estado es obligatorio";
    } elseif (!in_array($estado, $tipos_estado_permitidos)) {
        $errores[] = "El estado debe ser 'rechazada'";
    }
    ...
    if (empty($errores)) {
        try {
            $sql = $conexion->prepare("UPDATE ordenes_trabajo SET estado = :estado, motivo_rechazo = :motivo_rechazo WHERE id_orden = :id_orden");
            $sql->bindParam(':id_orden', $id_orden, PDO::PARAM_INT);
            $sql->bindParam(':motivo_rechazo', $motivo_rechazo, PDO::PARAM_STR);
        ...
        header("Location: ../mecanico/rechazar_orden_trabajo.php?id_orden=" . base64_encode($id_orden));
        exit();
    }
} else {
