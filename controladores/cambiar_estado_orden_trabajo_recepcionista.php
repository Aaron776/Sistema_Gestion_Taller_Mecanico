...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['id_orden']) && isset($_POST['estado']) ) {
    // Validar token CSRF
    ...
    if (empty($errores)) {
        try {
            $sql = $conexion->prepare("UPDATE ordenes_trabajo SET estado = :estado WHERE id_orden = :id_orden");
            $sql->bindParam(':id_orden', $id_orden, PDO::PARAM_INT);
        ...
        header("Location: ../recepcionista/cambiar_estado_orden.php?id_orden=" . base64_encode($id_orden));
        exit();
    }
} else {
