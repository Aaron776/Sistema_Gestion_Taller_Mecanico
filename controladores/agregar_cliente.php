...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['nombre']) && isset($_POST['apellido']) && isset($_POST['cedula']) && isset($_POST['telefono']) && isset($_POST['email']) && isset($_POST['direccion'])) {
    // Validar token CSRF
    ...
    if (empty($telefono)) {
        ...
        $errores[] = "El telefono debe tener 10 digitos y solo números";
    }
    ...
    // Verificar duplicidad de cliente
    if (empty($errores)) {
        try {
            $sql = $conexion->prepare("SELECT * FROM clientes WHERE cedula_ruc = :cedula_ruc OR email = :email");
            $sql->bindParam(':cedula_ruc', $cedula, PDO::PARAM_STR);
            $sql->bindParam(':email', $email, PDO::PARAM_STR);
            $sql->execute();
            $cliente = $sql->fetch(PDO::FETCH_OBJ);
            if ($cliente) {
                $errores[] = "Ya existe un cliente con la misma cédula/RUC o email";
            }
        } catch (PDOException $e) {
            $errores[] = "Error al verificar la duplicidad del cliente: " . $e->getMessage();
        }
    }
    ...
    if (empty($errores)) {
        try {
            $sql = $conexion->prepare("INSERT INTO clientes (nombre, apellido, cedula_ruc, telefono, email, direccion) VALUES (:nombre, :apellido, :cedula_ruc, :telefono, :email, :direccion)");
            ...
            $id_cliente = $conexion->lastInsertId(); // Obtener el ID del cliente recién insertado
            ...
            if (isset($_SESSION['rol']) && in_array($_SESSION['rol'], ['recepcionista', 'mecanico'])) {
                ...
                $mensaje = "El usuario " . $_SESSION['nombre'] . " " . $_SESSION['apellido'] . " ha agregado al cliente con ID: " . $id_cliente;
                $tipo = 'success';
            ...
            $errores[] = "Error al agregar el cliente. Intente nuevamente.";
        }
    } else {
        $_SESSION['errores'] = $errores;
        $_SESSION['datos_cliente_form'] = $_POST; // Guardar datos para repoblar el formulario
        header("Location: ../recepcionista/agregar_cliente.php");
        exit();
    }
} else {
