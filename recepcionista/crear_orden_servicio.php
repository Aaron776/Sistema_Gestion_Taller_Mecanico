...
// Si hay errores de validación, los valores ingresados se recuperan de la sesión ($datos_orden_servicio)
// y se asignan a los atributos 'value' de los inputs para que el usuario no tenga que reescribirlos.
$datos_orden_servicio = isset($_SESSION['datos_orden_servicio_form']) ? $_SESSION['datos_orden_servicio_form'] : []; // Recuperar datos del formulario 
unset($_SESSION['datos_orden_servicio_form']); // Limpiar datos del formulario
?>
<style>
...
<form id="nuevaOrdenForm" method="POST" action="../controladores/crear_orden_servicio.php">
                ...
                <div class="form-section">
                        ...
                        <div class="form-group">
                            ...
                            <select class="form-control" id="tipo_servicio" name="id_servicio" required>
                                ...
                                <?php foreach ($lista_servicios as $item) { ?>
                                    <!-- Se agrega el atributo data-precio para guardar el precio base del servicio en el HTML -->
                                    <option value="<?php echo $item->id_servicio; ?>" data-precio="<?php echo $item->precio_base; ?>"><?php echo $item->nombre_servicio; ?></option>
                                <?php } ?>
                            </select>
                        </div>
                        <div class="form-group">
                            ...
                            <select class="form-control" id="id_orden" name="id_orden" required>
                                ...
                                <?php foreach ($lista_ordenes as $item) { ?>
                                    <option value="<?php echo $item->id_orden; ?>"><?php echo "OT-" . $item->id_orden; ?> - <?php echo $item->nombre_cliente; ?> - <?php echo $item->placa_vehiculo; ?></option>
                                <?php } ?>
                            </select>
                        </div>
                    </div>
                    ...
                    <div class="form-row">
                        <div class="form-group">
                            ...
                            <input type="number" class="form-control" id="cantidad" min="1" value="<?= isset($datos_orden_servicio['cantidad']) ? htmlspecialchars($datos_orden_servicio['cantidad']) : '1' ?>" name="cantidad" required>
                        </div>
                        <div class="form-group">
                            <label for="precio" class="required">Precio Unitario *</label>
                            <input type="number" class="form-control" step="0.01" id="precio" min="0" value="<?= isset($datos_orden_servicio['precio_unitario']) ? htmlspecialchars($datos_orden_servicio['precio_unitario']) : '0.00' ?>" name="precio_unitario" required>
                        </div>
                    </div>
                </div>
            </div>
    ...
    <div class="form-actions">
        ...
        <div class="form-actions-right">
            ...
            <button type="submit" class="btn btn-success" id="btnCrearOrden">
                <i class="fas fa-check-circle"></i> Crear Orden de Servicio
            </button>
        </div>
    </div>
</form>
...
<script src="../js/recepcionista/crear_orden_servicio.js"></script>
