...
$sql_completadas = $conexion->prepare("SELECT COUNT(*) as total FROM ordenes_trabajo WHERE id_usuario_asignado = :id_mecanico AND estado = 'finalizado' AND DATE(fecha_entrega) = CURDATE()");
...
<div class="card">
    <div class="card-header">
        <h2 class="card-title">Lista de Órdenes de Trabajo Asignadas</h2>
        <div class="search-container" style="flex-wrap: wrap; gap: 10px;">
            <a href="../ReportesPDF/reporteOrdenesTrabajoAsignadas.php" class="btn btn-danger">
                <i class="fas fa-file-pdf"></i> PDF
            </a>
            <a href="../ReportesExcel/reporteOrdenesTrabajoAsignadas.php" class="btn btn-success">
                            ...
                            <tr>
                                ...
                                <td>
                                    <div style="display: flex; gap: 5px;">
                                        <a href="editar_estado_orden_trabajo_asignada.php?id_orden=<?php echo htmlspecialchars(base64_encode($item->id_orden)); ?>&id_cliente=<?php echo htmlspecialchars(base64_encode($item->id_cliente)); ?>" type="button" class="btn-action btn-action-primary">
                                            <i class="fas fa-edit"></i> Editar Estado
                                        </a>
                                        <a href="rechazar_orden_trabajo.php?id_orden=<?php echo htmlspecialchars(base64_encode($item->id_orden)); ?>" type="button" class="btn-action btn-action-danger">
...
<!-- Formulario Oculto para Rechazar Orden -->
<form id="formRechazar" method="POST" action="../controladores/cambiar_estado_orden_trabajo_asignados.php" style="display:none;">
    <input type="hidden" name="id_orden" id="rechazar_id_orden">
    <input type="hidden" name="id_cliente" id="rechazar_id_cliente">
    <input type="hidden" name="estado" value="rechazado">
    <input type="hidden" name="csrf_token" value="<?php echo $_SESSION['csrf_token']; ?>">
</form>

<script>
    function rechazarOrden(idOrden, idCliente) {
        if (confirm('¿Estás seguro de que deseas rechazar esta orden de trabajo? Esta acción no se puede deshacer.')) {
            document.getElementById('rechazar_id_orden').value = idOrden;
            document.getElementById('rechazar_id_cliente').value = idCliente;
            ...
            if (estado) {
                ...
                switch (estado) {
                    ...
                    case 'cancelled':
                        ...
                        claseEstado = 'status-cancelled';
                        break;
                    case 'rechazada':
                        estadoTexto = 'Rechazada';
                        claseEstado = 'status-rechazada';
                        break;
                }

                previewStatus.textContent = estadoTexto;
                previewStatus.className = `status-badge ${claseEstado}`;
