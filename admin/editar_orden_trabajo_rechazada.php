...
if (!isset($id_orden) || !is_numeric($id_orden) || !isset($id_cliente) || !is_numeric($id_cliente)) {
    header("Location: lista_ordenes_trabajo_rechazadas.php");
    exit();
}
...
$sql = $conexion->prepare("SELECT id_orden,id_cliente,estado,fecha_creacion,id_usuario_asignado FROM ordenes_trabajo WHERE id_orden = :id_orden AND id_cliente = :id_cliente");
...
// Obtener lisatdo de usuarios con rol mecanico
...
<div class="card">
    <div class="card-header">
        <h2 class="card-title">Reasignar Orden Rechazada</h2>
    </div>
    <div class="card-body">
        <form id="estadoForm" class="status-form" method="POST" action="../controladores/cambiar_estado_orden_trabajo_rechazada.php">
            ...
            <div class="form-group">
                <label for="estadoOrden">Seleccionar Mecanico</label>
                <select class="form-control" id="estadoOrden" name="mecanico" required>
                    <option value="">Seleccionar estado</option>
                    <?php foreach ($mecanicos as $item) { ?>
                        <option value="<?php echo $item->id_mecanico ?>" <?php echo $item->id_mecanico == $orden->id_usuario_asignado ? 'selected' : '' ?>><?php echo $item->nombre_mecanico ?></option>
                    <?php } ?>
                </select>
            </div>
