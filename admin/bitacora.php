...
$sql_count = $conexion->prepare("SELECT COUNT(*) as total FROM notificaciones");
$sql_count->execute();
$total_registros = $sql_count->fetch(PDO::FETCH_OBJ)->total;
$total_paginas = ceil($total_registros / $registros_por_pagina);

// Obtener listado de acciones del sistema
$sql = $conexion->prepare("SELECT id_notificacion, notificaciones.fecha, notificaciones.titulo, notificaciones.mensaje, usuarios.nombre, usuarios.apellido, usuarios.rol FROM notificaciones INNER JOIN usuarios ON notificaciones.id_usuario_origen=usuarios.id_usuario ORDER BY fecha DESC LIMIT :limit OFFSET :offset");
$sql->bindValue(':limit', (int)$registros_por_pagina, PDO::PARAM_INT);
$sql->bindValue(':offset', (int)$offset, PDO::PARAM_INT);
$sql->execute();
$lisatdo_acciones = $sql->fetchAll(PDO::FETCH_OBJ);
...
<div class="orders-summary">
    <div class="summary-card">
        <h3><?php echo htmlspecialchars($total_registros); ?></h3>
...
<div class="card">
    <div class="card-header">
        <h2 class="card-title">Bitácora de Actividades del Sistema</h2>
        ...
        <div class="table-responsive">
            <table class="table" id="ordenesTable">
                ...
                <tbody>
                    ...
                    <?php else : ?>
                        <?php foreach ($lisatdo_acciones as $item) { ?>
                            <tr>
                                <td><strong>#<?php echo htmlspecialchars($item->id_notificacion); ?></strong></td>
                                <td><?php echo htmlspecialchars(date('d/m/Y H:i', strtotime($item->fecha))); ?></td>
                                <td>
                                    <div><?php echo htmlspecialchars($item->nombre . ' ' . $item->apellido); ?></div>
                                    <small style="color: var(--gray);"><?php echo htmlspecialchars(ucfirst($item->rol)); ?></small>
                                </td>
                                <td><?php echo htmlspecialchars($item->titulo); ?></td>
                                <td><?php echo htmlspecialchars($item->mensaje); ?></td>
                                <td>
                                    <form action="../controladores/eliminar_notificacion.php" method="POST" class="formEliminar">
                                        <input type="hidden" name="id_notificacion" value="<?php echo htmlspecialchars($item->id_notificacion); ?>">
